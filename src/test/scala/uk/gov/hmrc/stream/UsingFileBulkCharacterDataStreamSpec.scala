...
class UsingFileBulkCharacterDataStreamSpec extends WordSpecLike with Matchers {
  "Processing a stream of data" should {
    "execute a function for each deliminator segregated entity found in the data source" in new UsingFileBulkCharacterDataStream[String] {
      ...
      processEntitiesWith(';', sourceData(new File("someLocation/file.ext")))(counterInstance.keepTrackOfCallCount)
