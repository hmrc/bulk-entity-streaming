...
class UsingReactiveBulkCharacterDataStreamSpec extends WordSpecLike with Matchers with FutureAwaits with DefaultAwaitTimeout with Eventually {
  ...
  "Processing a stream of data" should {
    "execute a function for each deliminator segregated entity found in the data source" in new UsingReactiveBulkCharacterDataStream[String] {
      ...
      await(processEntitiesWith(';', "someLocation")(counterInstance.keepTrackOfCallCount))
