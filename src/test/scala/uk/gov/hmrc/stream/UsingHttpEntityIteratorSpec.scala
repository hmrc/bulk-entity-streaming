...
class UsingHttpEntityIteratorSpec extends WordSpecLike with Matchers {

  "Processing a stream of data" should {
    "execute a function for each deliminator segregated entity found in the data source" in new UsingHttpEntityIterator[String] {
      ...
      bulkEntities(';', sourceData("someLocation")).foreach(counterInstance.keepTrackOfCallCount)

      counterInstance.counter shouldBe 4
    }

    "execute a function for each deliminator segregated entity found in the data source with a trailing entity" in new UsingHttpEntityIterator[String] {
      override implicit def convert(input: String): String = input
